...
interface ConnectionWithProfile extends Models.Document {
  ...
  receiverId: string;
  status: string;
  conversationId?: string; // Added to interface to match your schema
  friendSince?: string;
  otherUserProfile?: {
    username: string;
    profilePictureUrl: string;
...
export default function DiscoveryPage() {
  ...
  const generateConversationId = (userId1: string, userId2: string) => {
    // Sort IDs to ensure consistent conversationId regardless of who initiates
    const sortedIds = [userId1, userId2].sort();
    return `${sortedIds[0]}_${sortedIds[1]}`;
  };

  // Handle navigation to chat
  const handleChatNavigation = (connection: ConnectionWithProfile) => {
    ...
    // Navigate to the chat page with the generated conversationId
    ...
    const init = async () => {
      try {
        ...
        const [profileRes, connectionsRes] = await Promise.all([
          ...
          databases.listDocuments(DATABASE_ID, CONNECTIONS_COLLECTION_ID, [
            Query.or([
  ...
  const fetchAllConnections = async () => {
    if (!currentUserId) return;
    try {
      const allConnections = await databases.listDocuments(DATABASE_ID, CONNECTIONS_COLLECTION_ID, [
        Query.or([Query.equal('senderId', currentUserId), Query.equal('receiverId', currentUserId)])
      ]);
      ...
      for (const doc of allConnections.documents) {
        ...
        const profileQuery = await databases.listDocuments(DATABASE_ID, PROFILES_COLLECTION_ID, [
        ...
        // Generate conversationId for accepted connections
        let conversationId = connection.conversationId;
        if (connection.status === 'accepted' && !conversationId) {
          conversationId = generateConversationId(currentUserId, otherUserId);
        }
        
        const enriched = {
          ...connection,
          conversationId, // Use existing or generated
          isIncoming,
          otherUserProfile: {
  ...
  const handleResponse = async (id: string, status: 'accepted' | 'declined') => {
    ...
    // When accepting, we need to generate and store the conversationId
    const updateData: any = { status };
    
    if (status === 'accepted') {
      // Find the connection to get the other user's ID
      const connection = [...incomingRequests, ...sentRequests].find(c => c.$id === id);
      if (connection) {
        const otherUserId = connection.isIncoming ? connection.senderId : connection.receiverId;
        updateData.conversationId = generateConversationId(currentUserId, otherUserId);
        updateData.friendSince = new Date().toISOString();
      }
    }

    await databases.updateDocument(DATABASE_ID, CONNECTIONS_COLLECTION_ID, id, updateData);
    fetchAllConnections();
  };
  ...
  return (
    <div className="min-h-screen bg-[#1A0B2E] text-white flex overflow-hidden">
      ...
      {/* SIDEBAR - Restored exactly as provided */}
              ...
              {filteredDiscovery.length === 0 ? (
                ...
                filteredDiscovery.map((profile, i) => (
                  <Link key={profile.$id} href={`/profile/${profile.$id}`}>
                    <div className="group flex flex-col items-center animate-[zoomIn_0.5s_ease-out_forwards]" style={{animationDelay: `${i*0.05}s`}}>
                      <div className="relative w-40 h-40 md:w-52 md:h-52 rounded-full p-1.5 bg-gradient-to-tr from-pink-500 via-purple-500 to-orange-400 transition-transform group-hover:scale-105 group-hover:rotate-6 shadow-2xl">
                        <div className="w-full h-full rounded-full overflow-hidden border-[6px] border-[#1A0B2E] relative">
                          <Image src={profile.profilePictureUrl} alt={profile.username} fill className="object-cover grayscale-[0.3] group-hover:grayscale-0 transition-all duration-700" priority={i < 4} />
                        </div>
                      ...
                      <p className="mt-2 text-[10px] uppercase tracking-[0.2em] font-black text-white/30 bg-white/5 px-4 py-1.5 rounded-full">{profile.location}</p>
                    </div>
                  </Link>
                )
              ))}
               ...
               {(activeMatchTab === 'incoming' ? incomingRequests : activeMatchTab === 'sent' ? sentRequests : acceptedConnections).map((conn) => (
                  <div key={conn.$id} className="bg-[#120621] border border-white/5 rounded-[2.5rem] p-8 hover:bg-white/[0.04] transition-all relative overflow-hidden group">
                     <div className="flex items-center gap-5 mb-8">
                        <div className="relative">
                          <img src={conn.otherUserProfile?.profilePictureUrl} className="w-20 h-20 rounded-2xl object-cover border border-white/10 grayscale group-hover:grayscale-0 transition-all" alt="Avatar" />
                        ...
                        <div>
                           ...
                           <p className="text-[9px] text-white/30 tracking-[0.2em] uppercase font-bold">{conn.otherUserProfile?.location}</p>
                        </div>
                     </div>
